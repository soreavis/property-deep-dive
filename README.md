...
**Process (1)** — `--language` (foreign-buyer deed-language rule + sworn-translator regime + Hague Apostille / POA chain + diagnostic-doc language + sworn-translation cost bands)
...
→ detects FR by postcode, runs all 26 sections, prints to terminal
...
└── skills/property-deep-dive/        # the skill payload (everything plugin hosts ship)
    ...
    ├── shared/                       # 38 universal layer files (~13,200 lines)
...
**Repo total**: 169 markdown files, ~79,900 lines (skill content + community / governance files + CHANGELOG) · 35 YAML / JSON config files (27 workflows + 5 issue forms + dependabot + labels + labeler).
