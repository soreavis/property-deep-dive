...
**Sub-section extensions** (13 — auto-loaded alongside the parent flag, no separate invocation): `shared/mains-reliability.md` loads with `--mains` (grid + SAIDI/SAIFI + outage band + load-shedding regime); `shared/transport-noise.md` loads with `--traffic` (flight / rail / wind-turbine acoustic exposure — strategic noise maps + WHO 2018 thresholds + wind dB statutes; map-band granularity, classify-the-system spine, honest OSM-proximity tail); `shared/finance-banking.md` loads with `--finance` (tax-ID prerequisite chain + FATCA acceptance bucket per country + expat-banking arms); `shared/notary-forced-heirship.md` loads with `--notary` (civil-law forced shares vs common-law testamentary freedom vs Sharia, Brussels IV); `shared/notary-deed-anatomy.md` loads with `--notary` (clause-level forensic checklist of the sale-deed text — prescribed-form / mandatory-content anchors UK TR1 · ES LH art.9 · FR · DE · AT · CH · IT · NL · Torrens + 11-item universal red-flag existence checklist; decision-support, not legal advice); `shared/risks-build-quality.md` loads with `--risks` (statutory new-build warranty regimes + off-plan deposit-protection mechanisms — pairs strongly with `--type=off-plan`); `shared/digital-nomad-healthcare.md` loads with `--digital-nomad` (working-age 25-65 healthcare access for DN-visa holders — companion to `--retirement` for retirement-age 65+); `shared/rental-yield-delta.md` loads with `--rental` (gross-vs-net yield delta + neighborhood STR-zoning / moratorium overlay); `shared/scams-postcompletion.md` loads with `--scams` (post-completion fraud register — title hijack, utility/identity, equity-stripping); `shared/journey-sellrent.md` loads with `--journey=foreign-buyer` (sell-vs-rent decision overlay); `shared/integrity-remorse.md` loads with `--integrity` (buyer's-remorse / cooling-off cross-check); `shared/finance-crossborder.md` loads with `--finance` (cross-border mortgage / SPV / crypto-funding paths); `shared/exit-seller-withholding.md` loads with `--exit` (non-resident-seller withholding / tax-clearance completion-gate). The gap-#5 / gap-#4 fold-ins (strata-governance, right-of-way, leasehold, connect-from-scratch utilities, flood-insurability carve-out, JP universal 国土法/農地法 content) are in-file sub-blocks within existing section files (`property-management.md`, `property-types.md`, `insurance.md`, `countries/jp/playbook.md`), not separate auto-loaded extensions.
...
121 fully-populated country playbooks. Browse [`skills/property-deep-dive/countries/`](../skills/property-deep-dive/countries/) or see the [country matrix in README](../README.md#country-support--121-fully-populated).
