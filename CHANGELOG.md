...
- **Maintenance automation Tier-1 batch** — three improvements that turn detection-only workflows into proactive ones:
  ...
  - **Auto-PR for permanent URL redirects** — new weekly workflow `auto-pr-redirects.yml` (Tuesday 09:00 UTC, post-url-liveness) detects 301/308 redirects in country playbooks where the new URL stays on the same registered root domain, auto-replaces, and opens a PR. Cross-domain redirects skipped for human review. Capped at 20 replacements per PR for safety. Logic in `scripts/find_redirects.py`.
